...
function Signup() {
  ...
  const handleSubmit = async e => {
    ...
    setMsg("");
    try {
      const res = await fetch(`${BASE_URL}/store/register/`, {
        method: "POST",
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <div className="max-w-md w-full bg-white p-6 rounded shadow">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input name="password1" type="password" onChange={handleChange} value={form.password1} placeholder="Password" required className="w-full p-2 border rounded"/>
